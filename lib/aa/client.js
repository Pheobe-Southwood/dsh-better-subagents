...
export function createAaClient(options) {
  ...
  async function runLookup(provider, model) {
    ...
    // answered; saying so beats claiming AA never measured the model. This is
    // tested BEFORE the deadline: a `timeoutMs` of 0 is an already-expired
    // budget, so the abort check below would otherwise report a zero-budget
    // configuration as "lookup timed out after 1ms".
