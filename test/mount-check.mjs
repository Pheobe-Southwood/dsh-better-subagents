 ...
 *
 * Three parity rules keep the mount honest, and all three are asserted here:
 *
 *   - the package declares `dsh.bundle.patch`, and `files` ships that patch, so
 *     an npm publish cannot drop the only mount row;
 *   - the patch contributes exactly one `insert:` row;
 *   - that row's `name` is this package's name, and its `id` equals the `name`
 *     the host half exports, so the row and the plugin are recognizably the same
 *     thing.
 *
 * The patch is parsed by a shape-specific reader rather than a YAML dependency:
 * this package deliberately has no dependencies at all, and the file's shape is
 * one `insert:` list of `id`/`name` scalars (see the file's own comments).
 *
...
assert.equal(manifest.name, 'dsh-better-subagents', 'package name is the published id')
...
assert.deepEqual(
  ...
  'the entry exports exactly the Cordis plugin surface',
)
assert.equal(entry.name, 'better-subagents', 'the entry name is the row id')
assert.deepEqual(entry.inject, ['tools'], 'tools is the only hard dependency')
...
assert.ok(modules.length >= 8, `expected the lib modules to be present, saw ${modules.join(', ')}`)
...
for (const module of ['aa/client.js', 'aa/parse.js']) {
  const loaded = await import(new URL(`lib/${module}`, root))
  assert.ok(loaded !== null && typeof loaded === 'object', `lib/${module} must import cleanly`)
}
...
const readScalar = (key) => {
  // A row such as `- id: better-subagents` carries its key after a sequence
  // dash, so the dash is part of the scalar line rather than an indentation.
...
// --- the README must warn about the duplicate-id boot failure ---------------

const readme = await readFile(new URL('README.md', root), 'utf8')
assert.match(
  readme,
  /duplicate loader entry id/,
  'README must document that a hand-written duplicate row is a boot failure',
)

console.log(`MOUNT CHECK: ALL PASS (${modules.length + 2} lib modules, row id=${rowId}, name=${rowName})`)
